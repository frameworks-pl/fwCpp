...
const std::string SILicense::mLicPassword = "ZDGMSBHD50003";
const CString SILicense::mLicDateTimeMask = _T("Y-m-d H:i");
...
SILicense::SILicense(const CString& pName, const CString& pEmail, 
...
SILicense::generate() {
  ...
  try {
    ...
    fw::core::ByteBuffer sha256_encoded_hex = SIEncDec::hexEncode(sha256.getBuffer(), CryptoPP::SHA256::DIGESTSIZE);
   ...
   // CString license_full_xml(UTF82Unicode(std_license_full_xml.c_str()));
    ...
    fw::core::ByteBuffer xml_encrypted_encoded = SIEncDec::hexEncode(xml_encrypted.getBuffer(), xml_encrypted.getLength());

    mRawLicense = std::string((const char*)xml_encrypted_encoded.getBuffer(), xml_encrypted_encoded.getLength());

  }
  catch (sicrypt::SICryptException) 
  {
    mLicenseState = LIC_BAD;
  }
...
SILicense::read() {
  ...
  try {
    ...
    fw::core::ByteBuffer decoded_lic = SIEncDec::hexDecode((BYTE*)mRawLicense.c_str(), (int)mRawLicense.size());
    SISHA256 sha256;
    ...
    fw::core::ByteBuffer checksum_decrypted = SIEncDec::hexDecode((BYTE*)checksum.c_str(), (int)checksum.size());
    //2. initiating sha256 with the decrypted checksum
    SISHA256 checksum_sha256;
...
bool
SILicense::save(const CString& pFilePath) {
...
bool
SILicense::load(const CString& pFilePath) {
...
SYSTEMTIME 
SILicense::getLicenseGenerationTime() {
...
SYSTEMTIME 
SILicense::getFullLicenseTimeLimitation() const {
...
bool 
SILicense::parse(const std::string& pLicRaw) {
...
int 
SILicense::countDaysPassed(const SYSTEMTIME& pStartDate) {
...
bool 
SILicense::verifyAppIdent(const CString& pAppIdent) {
