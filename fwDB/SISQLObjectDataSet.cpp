  ...
  if (pSQLObject)
  {
    ...
    if (pRC)
    {
      ...
      if (sClassToCreate != sClassOfDataSet)
        throw sidb::SIDBException(_T("Trying to add an object of wrong type."));
...
/*                if (INVALID_SQL_ID != pSQLObject->getSQLID())
    ...
    else throw sidb::SIDBException(_T("The type object is unknown."));

  }
  else
  {
    throw sidb::SIDBException(_T("Trying to add non existing object to collection."));
  }
  ...
  if (false == bDoNotNotify)
  {
    ...
    for (it = m_Listeners.begin(); it != m_Listeners.end(); it++)
      ((sidb::SISQLObjectDataSet_Listener*)(*it))->onObjectModified();
  }
...
const SISQLObject* SISQLObjectDataSet::getNextObject() const
{
  ...
  while (m_Iterator != m_ObjectList.end())
  {
    ...
    {
      const sidb::SISQLObject* pObject = m_Iterator->second;
      m_Iterator++;
      return pObject;
    }
    m_Iterator++;
